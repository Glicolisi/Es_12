...
public class Main {
    ...
    public static double exercise2(String mealType, double weight, double expectedScore) {
        ...
        if (mealType.contains("sandwich")){
            ...
            System.out.println(mealType+" not here");
        }
        if (yourMealScore<5){
            yourMealScore=5;
        }
        //modificato il codice base per aggiungere un caso non in elenco (pizza)
        if(!(mealType.contains("sandwich")||mealType.contains("soup"))){
            System.out.println("Cannot calculate score of "+mealType);
        } else{
